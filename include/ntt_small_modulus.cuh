...
template <uint32_t N, uint32_t Q_BITS = 32>
using SmallNTTValueFor =
    std::conditional_t<Q_BITS == 32, uint32_t, uint64_t>;

template <uint32_t N, uint32_t Q_BITS = 32>
struct SmallNTTModulus {
    static_assert(N == TFHEpp::lvl1param::n || N == TFHEpp::lvl2param::n,
                  "Unsupported small NTT length");
    static_assert(Q_BITS == 32 || Q_BITS == 64,
                  "Small NTT supports 32- or 64-bit torus moduli only");
    static constexpr SmallNTTValue P =
        Q_BITS == 32 ? small_ntt31::P : small_ntt::P;
    static constexpr SmallNTTValue P_MINUS_ONE =
        Q_BITS == 32 ? small_ntt31::P_MINUS_ONE : small_ntt::P_MINUS_ONE;
    static constexpr SmallNTTValue HALF_P =
        Q_BITS == 32 ? small_ntt31::HALF_P : small_ntt::HALF_P;
    static constexpr uint32_t WORDBITS =
        Q_BITS == 32 ? small_ntt31::WORDBITS : small_ntt::WORDBITS;
};
...
template <uint32_t length, uint32_t Q_BITS = 32>
struct SmallNTTParams {
    SmallNTTValueFor<length, Q_BITS>* forward_root;
    SmallNTTValueFor<length, Q_BITS>* inverse_root;
    SmallNTTValueFor<length, Q_BITS> n_inverse;
    bool initialized;
};

extern std::vector<SmallNTTParams<TFHEpp::lvl1param::n, 32>> g_small_ntt_params;
extern std::vector<SmallNTTParams<TFHEpp::lvl2param::n, 64>>
 ...
 */
template <uint32_t length = TFHEpp::lvl1param::n, uint32_t Q_BITS = 32>
...
constexpr bool USE_LOW_LDS_BOOTSTRAP =
    ...
    P::n == TFHEpp::lvl1param::n || sizeof(typename P::T) == 8;
#else
...
template <uint32_t length = TFHEpp::lvl1param::n, uint32_t Q_BITS = 32>
using CuNTTHandler = CuGPUFFTHandler<length, Q_BITS>;
...
template <uint32_t length = TFHEpp::lvl1param::n, uint32_t Q_BITS = 32>
using CuNTTHandler = CuSmallNTTHandler<length, Q_BITS>;
...
template <uint32_t N, uint32_t Q_BITS = 32>
using NTTValueFor = SmallNTTValueFor<N, Q_BITS>;
...
template <class P = TFHEpp::lvl1param>
constexpr uint32_t MEM4HOMGATE_WORK =
    (USE_PAIRED_NTT<P> ? 2 : 1) * P::n * sizeof(NTTValueFor<P::n, sizeof(typename P::T) * 8>);
...
constexpr uint32_t MEM4HOMGATE =
    USE_REGISTER_NTT_ACCUM<P>
        ? (MEM4HOMGATE_WORK<P> > MEM4HOMGATE_TLWE<P> ? MEM4HOMGATE_WORK<P>
                                                     : MEM4HOMGATE_TLWE<P>)
        : (P::k + 2) * P::n * sizeof(NTTValueFor<P::n, sizeof(typename P::T) * 8>);
...
extern __device__ NTTValueFor<TFHEpp::lvl2param::n, 64>* block_xai_fft_lvl02;
#endif
...
extern std::vector<NTTValueFor<TFHEpp::lvl2param::n, 64>*> xai_ntt_devs_lvl02;
extern std::vector<NTTValueFor<TFHEpp::lvl2param::n, 64>*> one_trgsw_ntt_devs_lvl02;
